...
def svg_overwrite(filename, commit_data, pr_data, repo_data, contrib_data, contrib_year_data, loc_data):
    ...
    Two vertical alignment columns are kept consistent across all three stat lines:
      * COL   - where the | and ( separators sit (the central divider)
      * WIDTH - the right edge, where the right-hand values and the closing ) line up,
                matching the length of the header dash line.
    The right-hand values are right-aligned to WIDTH with leader dots, and the ++/--
    figures are centered between ( and ) so they re-center on their own as numbers grow.
    """
    ...
    # Fixed character counts to the left of the separator (| or () on each line,
    # excluding the variable dots and data values:
    #   Line 1: '. Repos:' (8) + ' {Contributed: ' (15) + '} ' (2) = 25
    #   Line 2: '. Commits:' (10) + ' ' (1) = 11
    #   Line 3: '. Lines of Code:' (16) + ' ' (1) = 17
    FIXED_1, FIXED_2, FIXED_3 = 25, 11, 17
    ...
    col = max(
        ...
        FIXED_2 + len(commit_s) + MIN_DOTS,
        FIXED_3 + len(loc_net_s) + MIN_DOTS,
        COL_FLOOR,
    )

    def make_dots(budget):
        if budget <= 0: return ''
        if budget == 1: return ' '
        if budget == 2: return '. '
        return ' ' + ('.' * (budget - 2)) + ' '

    # Left side: dots that push the | / ( separators out to the divider column
    find_and_replace(root, 'repo_data',        repo_s)
    find_and_replace(root, 'repo_data_dots',   make_dots(col - FIXED_1 - len(repo_s) - len(contrib_s)))
    find_and_replace(root, 'contrib_data',     contrib_s)
    find_and_replace(root, 'commit_data',      commit_s)
    find_and_replace(root, 'commit_data_dots', make_dots(col - FIXED_2 - len(commit_s)))
    find_and_replace(root, 'loc_data',         loc_net_s)
    find_and_replace(root, 'loc_data_dots',    make_dots(col - FIXED_3 - len(loc_net_s)))

    # Right side of lines 1 & 2: right-align the value to WIDTH with leader dots.
    ...
    inner   = WIDTH - col - 2                                    # gap between ( and )
    content = len(loc_add_s) + 2 + 2 + len(loc_del_s) + 2        # 'X++, Y--'
    pad     = max(0, inner - content)
    find_and_replace(root, 'loc_add',  loc_add_s)
    find_and_replace(root, 'loc_del',  loc_del_s)
    find_and_replace(root, 'loc_lpad', ' ' * (pad // 2))
    find_and_replace(root, 'loc_rpad', ' ' * (pad - pad // 2))
...
def pr_getter(username):
    """
    Returns the total number of pull requests the user has opened
    """
    query_count('pr_getter')
    query = '''
    query($login: String!){
        user(login: $login) {
            pullRequests {
